...
/// Above the card, or in a column beside it. `top` is bottom-aligned to
/// `CAPTION_BASELINE` rather than top-aligned, so a headline that wraps grows
/// up into the empty frame instead of down into the card.
export const Caption: React.FC<{
  ...
  outAt?: number;
}> = ({ kicker, headline, note, where = "top", outAt }) => {
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          ...rise(frame, 4),
          marginTop: side ? 20 : 18,
          maxWidth: side ? COLUMN.width : 1400,
          fontSize: side ? 45 : 52,
          lineHeight: 1.14,
          fontWeight: 600,
          letterSpacing: -0.6,
      ...
      {note ? (
        <div
          style={{
            ...rise(frame, 10),
            marginTop: side ? 20 : 14,
            maxWidth: side ? COLUMN.width : 1180,
            fontSize: side ? 23 : 25,
